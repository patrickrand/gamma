...
func (sh ShellExecuter) Execute(cmd string, args ...string) (code int, message string) {
	...
	if err != nil {
		return sh.defaultErrorCode, "gamma.agent.ShellExecuter failed to execute the command: " + err.Error()
	}
	...
	if err := json.Unmarshal(data, &output); err != nil {
		return sh.defaultErrorCode, "gamma.agent.ShellExecuter failed to unmarshal the command's output: " + err.Error()
	}
